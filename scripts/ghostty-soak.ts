...
const fixtures = Array.from({ length: 256 }, (_, index) => new TextEncoder().encode(
  `\x1b[38;5;${index}magent ${index} λ 日本語 🙂 \x1b]8;;https://example.test/source/${index}\x1b\\source\x1b]8;;\x1b\\\x1b[0m\r\n`,
));
...
for (const artifact of ["ghostty-vt.wasm", "ghostty-vt-scalar.wasm"]) {
  it(`${artifact}: five models plateau under streaming, reflow, and presentation recycling`, async () => {
    ...
    try {
      ...
      const report = {
        artifact, models: models.length, updates: epochs * updatesPerEpoch * models.length,
