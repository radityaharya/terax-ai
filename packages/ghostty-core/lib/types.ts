...
export interface GhosttyWasmExports extends WebAssembly.Exports {
  ...
  ghostty_terminal_set_cursor_style(terminal: TerminalHandle, style: number): void;
  ghostty_terminal_set_cursor_blinking(
    terminal: TerminalHandle,
    blinking: boolean
  ): void;
  ...
  // Length-prefixed semantic event API
  ghostty_terminal_has_events(terminal: TerminalHandle): boolean;
  ghostty_terminal_read_events(
    terminal: TerminalHandle,
    bufPtr: number,
    bufLen: number
  ): number;
  ghostty_terminal_take_dropped_events(terminal: TerminalHandle): number;
}
 ...
 * Layout: four u32 values + palette[16] + cursor style/blink + 2 bytes padding.
 */
...
export type GhosttyTerminalEvent =
  | { readonly type: "bell" }
  | { readonly type: "title"; readonly title: string }
  | { readonly type: "pwd"; readonly uri: string }
  | {
...
export interface PackedViewport {
  readonly bytes: Uint8Array;
  readonly cellCount: number;
  readonly cellStride: 16;
