...
export class InputHandler {
  ...
  private onDataCallback: (data: TerminalInput) => void;
  ...
  constructor(
    ...
    onData: (data: TerminalInput) => void,
    _onBell: () => void,
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    try {
      ...
      // Keep the Ghostty key encoder output as bytes all the way to the PTY.
      if (encoded.length > 0) {
        this.onDataCallback(encoded);
        this.recordKeyDownData(event.key);
      }
    } catch (error) {
