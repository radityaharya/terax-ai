...
use super::super::workspace::WorkspaceRegistry;
...
pub fn resolve_authorized_root(
    resolved: &Path,
...
pub fn create_file(p: &Path) -> Result<(), String> {
    if p.exists() {
        return Err(format!("already exists: {}", p.display()));
    }
    std::fs::write(p, "").map_err(|e| {
        log::debug!("create_file({}) failed: {e}", p.display());
        e.to_string()
    })
}
...
pub fn create_dir(p: &Path) -> Result<(), String> {
    if p.exists() {
        return Err(format!("already exists: {}", p.display()));
    }
    std::fs::create_dir_all(p).map_err(|e| {
        log::debug!("create_dir({}) failed: {e}", p.display());
        e.to_string()
    })
}

/// Renames (or moves) a path. Refuses to overwrite an existing target.
pub fn rename_path(from_p: &Path, to_p: &Path) -> Result<(), String> {
    if !from_p.exists() {
...
/// All paths must already be workspace-resolved by the caller.
pub fn move_paths(
    from_p: &Path,
    to_p: &Path,
    root: &Path,
...
pub fn delete_path(p: &Path) -> Result<(), String> {
    remove_path(p).map_err(|e| {
        log::warn!("delete({}) failed: {e}", p.display());
        e.to_string()
    })
}

pub fn delete_batch(
    resolved: Vec<(String, std::path::PathBuf)>,
    root: &Path,
) -> FsDeleteBatchResult {
    let mut deleted = Vec::with_capacity(resolved.len());
    let mut failed = 0;
    for (path, resolved) in resolved {
        ...
        match result {
            ...
            Err(error) => {
                failed += 1;
                log::warn!("delete_batch({}) failed: {error}", resolved.display());
            }
        }
    }
...
mod tests {
    ...
    fn move_path(
        ...
        move_paths(from, to, &root, expected_conflict)
    }
    ...
    fn create_dir_builds_nested_chain_and_refuses_existing() {
        ...
        create_dir(&nested).expect("create dir");
        assert!(nested.is_dir());
        let err = create_dir(&nested).unwrap_err();
        assert!(err.contains("already exists"), "got: {err}");
    }
    ...
    fn rename_moves_and_never_overwrites() {
        ...
        rename_path(&from, &to).expect("rename");
        assert!(!from.exists());
        assert_eq!(std::fs::read(&to).unwrap(), b"payload");

        // Missing source is reported, not silently ignored.
        let err = rename_path(&from, &dir.path().join("c.txt")).unwrap_err();
        ...
        let err = rename_path(&to, &occupied).unwrap_err();
        assert!(err.contains("already exists"), "got: {err}");
    ...
    fn move_rejects_paths_outside_the_authorized_workspace() {
        ...
        let error = move_paths(&from, &to, &root, None).unwrap_err();
    ...
    fn delete_removes_file_then_dir_recursively() {
        ...
        delete_path(&f).expect("delete file");
        assert!(!f.exists());

        let sub = dir.path().join("sub");
        std::fs::create_dir_all(sub.join("inner")).unwrap();
        std::fs::write(sub.join("inner/y.txt"), b"y").unwrap();
        delete_path(&sub).expect("delete dir");
        assert!(!sub.exists());

        let err = delete_path(&dir.path().join("missing")).unwrap_err();
        assert!(!err.is_empty());
    }
    ...
    fn batch_delete_reports_each_success_and_failure() {
        ...
        let resolved = vec![
            ("first.txt".to_string(), first.clone()),
            ("missing.txt".to_string(), missing.clone()),
            ("second.txt".to_string(), second.clone()),
        ];

        let result = delete_batch(resolved, &root);

        assert_eq!(result.deleted, vec!["first.txt".to_string(), "second.txt".to_string()]);
    ...
    fn delete_does_not_follow_symlink_into_target() {
        ...
        delete_path(&link).expect("delete symlink");
...
pub fn copy_into(sources: &[String], dest: &Path) -> Result<(), String> {
