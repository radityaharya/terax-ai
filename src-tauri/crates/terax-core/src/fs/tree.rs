...
/// Lists immediate children of an already-resolved `root`. Dirs first, then
/// files, each sorted with `natural_cmp`. Callers resolve workspace paths
/// before calling; this function never touches the workspace registry.
pub fn read_dir_entries(
    ...
    let read = std::fs::read_dir(root).map_err(|e| {
        log::debug!("read_dir({}) failed: {e}", root.display());
        e.to_string()
    })?;
    ...
    let git_decorations = git_decorations && in_git_repo(root);
...
/// Lists immediate subdirectory names of an already-resolved `root`.
pub fn list_subdir_names(root: &Path, show_hidden: bool) -> Result<Vec<String>, String> {
    let read = std::fs::read_dir(root).map_err(|e| {
...
mod tests {
    ...
    fn non_cased_unicode_keeps_code_point_order() {
        assert_eq!(natural_cmp("나", "가"), Ordering::Greater);
    ...
    #[test]
    fn read_dir_lists_and_sorts() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("b.txt"), b"x").unwrap();
        std::fs::create_dir(dir.path().join("a-dir")).unwrap();
        let entries = super::read_dir_entries(dir.path(), false, false).unwrap();
